    ...
    <section id="invitese" v-else>
      <div class="container">
        ...
        <div class="userInfo">
          ...
          <div class="userName">
            {{ $t('loggedin-as', { user: $auth.user.global_name }) }},&nbsp;
          </div>
        ...
        <div v-if="auth_type === 1">
          <recaptcha />
          <v-btn size="x-large" class="joinBtn mt-5" @click="captcha(result.identifier)" block :loading="auth_loading"
        ...
        <div v-else-if="auth_type === 2">
          ...
          <v-btn type="submit" size="x-large" class="joinBtn mt-5" @click="email(result.identifier, $auth.user.email)"
            block :loading="auth_loading" :disabled="auth_loading">
            {{ $t('accept') }}
...
export default {
  ...
  methods: {
    checkLink: function (id) {
      this.$axios.get(`/api/link/check?id=${id}`, { progress: false }).then((res) => {
        this.loading = false;
    ...
    captcha: async function (id) {
      try {
        ...
        this.$axios.post('/api/link/authorization', {
          id,
          g_recaptcha: token
        }).then((res) => {
    ...
    getPresence: function (id) {
      this.$axios.get(`/api/presence?id=${id}`).then((res) => {

<template>
  <v-app>
    <v-container pa-5 mt-5>
      ...
      <div class="text-subtitle-1 mb-5" v-else>{{ $t('logged-in-as', { user: $auth.user.global_name }) }},
          ...
          <v-col>
            <v-card class="mb-5 mx-auto">
              ...
              <v-card-text class="pt-4">
                {{ $t('server-id') }}: {{ guild.id ? guild.id : 'N/A' }}
              </v-card-text>
              <v-card-text class="pt-4">
                {{ $t('bot-add-date') }}: {{ $dayjs(guild.joinedTimestamp).format('YYYY-MM-DD hh:mm:ss') }}
              </v-card-text>
            </v-card>
              ...
              <form
                ...
                <v-card-text>
                  ...
                  <v-select v-model="selected_method" :items="methods" :label="$t('auth-method-placeholder')"
                    :disabled="creating"></v-select>
                </v-card-text>
                <v-card-text>
                  {{ $t('validity-period') }}
                  <v-select v-model="selected_expires" :items="expires" :label="$t('validity-period-placeholder')"
                    :disabled="creating"></v-select>
                </v-card-text>
                <v-card-text>
                  {{ $t('role-to-add') }}
                  <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-select v-bind="attrs" v-on="on" v-model="selected_role" :items="roles" chips :label="$t('roles')"
                        :disabled="creating"></v-select>
                    </template>
                ...
                <v-card-text v-if="custom_link">
                  ...
                  <v-text-field type="text" :label="$t('invite-id')" @change="formValidation(custom_link_input)"
                    v-model.trim="custom_link_input" :disabled="creating" required></v-text-field>
                  <span v-if="validateMessage" :class="{ 'orange--text': warnMessage, 'green--text': !warnMessage }">{{
                  ...
                  <v-btn type="submit" color="primary" :loading="creating" :disabled="creating">
                    ...
                    <v-dialog v-model="dialog_create_error" width="auto" class="text-center">
                      <v-card>
                        ...
                        <v-card-actions>
                          <v-btn color="primary" outlined block @click="dialog_create_error = false">{{ $t('confirm')
                          }}</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
            ...
            <v-card class="pa-md-2 mb-5 mx-auto">
              ...
              <div v-else v-for="(data, i) in links" :key="i">
                <v-card-text>
                  <v-chip dark text-color="white" class="font-weight-bold">
                    ...
                    <v-tooltip top>
                      <template v-slot:activator="{ on, attrs }">
                        <span @click="copyCode(data.identifier)" @mouseout="reset" style="cursor: pointer;" v-bind="attrs"
                          v-on="on"><v-icon>mdi-content-copy</v-icon></span>
                      </template>
                      <span>{{ copyText }}</span>
                    </v-tooltip>
                  </v-chip>
                ...
                <v-card-text>
                  {{ $t('method') }}: {{ humanizeMethod(data.auth_method) }}
                </v-card-text>
                <v-card-text v-if="!data.no_expires">
                  {{ $t('expires') }}: {{ $dayjs(data.expiresAt).format('YYYY-MM-DD hh:mm:ss') }}
                </v-card-text>
                <v-card-text v-else>
                  {{ $t('expires') }}: {{ $t('no-expires') }}
                </v-card-text>
                <v-card-text>
              ...
              <v-dialog v-model="dialog_delete_confirm" width="auto" class="text-center">
                <v-card>
                  ...
                  <v-card-actions>
                    <v-btn color="primary" outlined @click="dialog_delete_confirm = false">{{ $t('cancel')
                    }}</v-btn>
  ...
  methods: {
    ...
    createCustomLink: function (gid, role, method, expire, custom_id, question, answer) {
      if (!this.selected_method || !this.selected_expires) {
        ...
        this.creating = true;
        this.$axios.post('/api/link/create/custom', {
          id: custom_id,
          gid,
          role,
          method,
    ...
    formValidation: function (id) {
      if (id) {
        ...
        this.$axios.get(`/api/link/create/custom/validation?id=${id}`, { progress: false }).then((res) => {
    ...
    deleteLink: function (id) {
      ...
      this.$axios.delete(`/api/link/delete?id=${id}`).then((res) => {
        this.deleting = false;
    ...
    getGuild: function (id) {
      this.loading = true;
      this.$axios.get(`/api/guild?id=${id}`).then((res) => {
        this.loading = false;
    ...
    getRoles: function (id) {
      this.$axios.get(`/api/guild/roles?id=${id}`).then((res) => {
        let roles = [];

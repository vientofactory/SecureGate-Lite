<template>
  <v-app>
    <v-container pa-5 mt-5>
      ...
      <div class="text-subtitle-1 mb-9" v-else>{{ $t('logged-in-as', { user: $auth.user.global_name }) }},
      ...
      <div v-else>
        <v-row>
          <v-col v-for="(data, i) in guilds" :key="i">
            <v-card width="300px">
              <v-img :src="getGuildIcon(data.id, data.icon)" class="white--text align-end"
                gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)" height="100px"></v-img>
...
export default {
  ...
  methods: {
    ...
    isInvited: function (id) {
      this.$axios.get(`/api/invited?id=${id}`, { progress: false }).then((res) => {
